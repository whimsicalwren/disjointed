...
public class SlimPlayerRagdoll implements Ragdoll<RagdollSlots.Humanoid> {

    private final EnumMap<RagdollSlots.Humanoid, Long> pieces;
    private final UUID uuid;
    public final String username;

    public SlimPlayerRagdoll() {
        this(null);
    }

    public SlimPlayerRagdoll(String username) {
        this.pieces = new EnumMap<>(RagdollSlots.Humanoid.class);
        this.uuid = UUID.randomUUID();
        this.username = username;
    }

    public SlimPlayerRagdoll(UUID uuid, String username, EnumMap<RagdollSlots.Humanoid, Long> pieces) {
        this.pieces = pieces;
        this.uuid = uuid;
        this.username = username;
    }
    ...
    public void createJoints(ServerLevel level) {
        ...
        VSSphericalJoint torsoToLeftArm = createRagdollJoint(torso, getSlot(RagdollSlots.Humanoid.LEFT_ARM), new Vector3d(pxToBlocks(4), pxToBlocks(6), 0), new Vector3d(pxToBlocks(-1.5f), pxToBlocks(6), 0));
        VSSphericalJoint torsoToRightArm = createRagdollJoint(torso, getSlot(RagdollSlots.Humanoid.RIGHT_ARM), new Vector3d(pxToBlocks(-4), pxToBlocks(6), 0), new Vector3d(pxToBlocks(1.5f), pxToBlocks(6), 0));
    ...
    public static SlimPlayerRagdoll create(ServerLevel level, Vector3d pos, String username, boolean isStatic) {
        SlimPlayerRagdoll ragdoll = new SlimPlayerRagdoll(username);
        ...
        ragdoll.addSlot(RagdollSlots.Humanoid.LEFT_ARM, shipWorld.createBody(createRagdollPieceData(level, armSlim(), new Vector3d(pos).add(pxToBlocks(5.5f), 0, 0), 1000, isStatic)));
        ragdoll.addSlot(RagdollSlots.Humanoid.RIGHT_ARM, shipWorld.createBody(createRagdollPieceData(level, armSlim(), new Vector3d(pos).add(pxToBlocks(-5.5f), 0, 0), 1000, isStatic)));
